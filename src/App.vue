  ...
  <div class="demo-page">
    <div class="demo-head">
      <div>
        ...
        <p>仅包含中间 Three.js 交互区域，周边指标大屏可由现有平台承载。</p>
      </div>
      <div class="switches">
    ...
    <div class="event-log">
      <b>最近事件：</b>{{ latestEvent || '点击楼栋、房间、设备点位或触发模拟告警后，会在这里显示事件。' }}
    </div>
  </div>
</template>
...
.demo-page { min-height: 100vh; padding: 22px; background: radial-gradient(circle at 50% 0%, rgba(0,245,255,.14), transparent 34%), linear-gradient(180deg,#020811,#01050a); }
.demo-head { max-width: 1500px; margin: 0 auto 16px; display: flex; align-items: center; justify-content: space-between; }
.demo-head h1 { margin: 0; font-size: 25px; letter-spacing: 2px; }
.demo-head p { margin: 8px 0 0; color: rgba(210,250,255,.65); }
